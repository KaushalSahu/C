...
struct node * createNode(char word[], struct node *child) {
	struct node *newNode;
...
void addValToNode(char word[], int pos, struct node *node, struct node *child) {
	int j = node->count;
...
/* split the node */
void splitNode (char word[], char **pword, int pos, struct node *node, struct node *child, struct node **newNode) {
	int median, j;
	...
	*newNode = (struct node *)malloc(sizeof(struct node));
	...
	if (pos <= MIN) {
		addValToNode(word, pos, node, child);
	} else {
		addValToNode(word, pos - median, *newNode, child);
	}
...
int setValueInNode(char word[], char **pval, struct node *node, struct node **child) {
	...
	if (setValueInNode(word, pval, node->link[pos], child)) {
		if (node->count < MAX) {
...
void insertion(char word[]) {
	...
	flag = setValueInNode(word, &i, root, &child);
	if (flag)
